...
async def mention_afk(mention):
    ...
    if ISAFK and mention.message.mentioned:
        ...
        if mention.sender_id not in USERS or chat_title not in USERS:
            if AFKREASON:
                await mention.reply(
                    f"**Lagi bikin dosa** (Since: {afk_str})"
                    f"\nReason: `{AFKREASON}`."
                )
            else:
                await mention.reply(
            ...
            if USERS[mention.sender_id] % randint(2, 4) == 0:
                if AFKREASON:
                    await mention.reply(
                        f"**Lagi ngobrol sama malaikat** (Since: {afk_str})"
                        f"\nReason: `{AFKREASON}`."
                    )
                else:
                    await mention.reply(
    ...
    if (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(
                        f"**Cuci dulu dosa** (Since: {afk_str})"
                        f"\nReason: `{AFKREASON}`."
                    )
                else:
                    await sender.reply(
                        f"**Lagi tobat dulu** (Since: {afk_str})"
                        "\n"
                    )
                ...
                if USERS[sender.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await sender.reply(
                            "**Cuci dulu dosa** (Since: {afk_str})"
                            f"\nReason: `{AFKREASON}`."
                        )
                    else:
                        await sender.reply(
                            "**Lagi tobat dulu** (Since: {afk_str})"
                            "\n"
                        )
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit("**Off ada pendosa!**" f"\nReason: `{string}`")
    else:
        await afk_e.edit("**Off ada pendosa!**")
    if BOTLOG:
        await afk_e.client.send_message(BOTLOG_CHATID, "#AFK\nGua afk dulu ada pendosa!")
    ISAFK = True
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.edit("**Assamulaikum tobat nya udah!**")
